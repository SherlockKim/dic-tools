...
//        Elements elements = new Elements();
        Elements elements = new Elements();
        for (int z = 1; z <= 10; z++) {
            String searchUrl = "http://www.boohee.com/food/view_menu" + "?page=" + z;
            HtmlPage page = WebClientUtil.getPage(webClient, searchUrl);
            String sort = Jsoup.parse(page.asXml()).select(".group-list li").get(10).text();
...
//                int elementIndex = (z-1)*10;
            for (int elementIndex = 0; elementIndex < srcList.size(); ++elementIndex) {
...
//                    System.out.println(detailElement.select(".content dl[class!=header] dd span[class=dd] ").eachText()+"999999999999");
                ...
                if (0 == list.size() % flushRows) {
                    ...
                    writeExcel("d:/tmp/test01/薄荷健康" + 1000 + "_" + 1100 + ".xlsx", 0, "食品库" + 1100, list);
                }
//                cnt++;
            }
